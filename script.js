...
async function uploadImage() {
  ...
  const { error: insertError } = await supabaseClient.from("ui_feedback").insert({
    user_id: user.id,
    image_url: imageUrl,
...
async function loadHistory() {
  const user = (await supabaseClient.auth.getUser()).data.user;
  const { data, error } = await supabaseClient.from("ui_feedback").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
